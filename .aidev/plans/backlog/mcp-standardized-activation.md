...
Sistema de detecção automática de stack e ativação padronizada de MCP servers em projetos novos ou existentes. Atualmente, MCPs são instalados manualmente via edição direta do `.mcp.json`, sem workflow do orquestrador para detectar a stack do projeto e configurar os servers apropriados automaticamente.
